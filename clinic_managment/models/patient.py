...
class Patient(models.Model):
    ...
    confirmed_appointment_ids = fields.One2many(
        'clinic.appointment', 'patient_id',
        string="Medical History",
        domain=[('status', '=', 'confirmed')]
    )
    consultation_ids = fields.One2many(
        'clinic.consultation', 'patient_id',
        string="Consultations History"
    )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            # Only create partner if not already linked
    ...
    def action_grant_access(self):
        for rec in self:
            ...
            if not user:
                ...
                # Create user WITHOUT groups — Odoo 19 does not support
                # groups_id in res.users.create(). Assign groups after.
                user = Users.with_company(company.id).with_context(
            ...
            # Assign portal, remove internal — write on res.groups (Odoo 19 compatible)
            user.sudo().write({'group_ids': [(4, portal_group.id), (3, internal_group.id)]})
